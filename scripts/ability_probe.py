...
def analyze_decisions(
    ...
    iterator. It does not decode new protocol semantics. The raw decision, opcode list,
    special codes and already-decoded state deltas are exposed so a mechanic can be
    proven from the same evidence pipeline before it is promoted to exact search.
    """
