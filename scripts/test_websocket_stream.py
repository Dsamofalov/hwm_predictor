...
def take(s,buffer,n):
    ...
    while len(data)<n:
        chunk=s.recv(n-len(data));assert chunk,"websocket closed while reading frame";data.extend(chunk)
    return bytes(data[:n]),bytes(data[n:])
def recv_frame(s,buffer=b""):
    h,buffer=take(s,buffer,2);assert len(h)==2 and (h[0]&0x0f)==1,h
    n=h[1]&0x7f
    if n==126:
        ext,buffer=take(s,buffer,2);n=int.from_bytes(ext,"big")
    elif n==127:
        ext,buffer=take(s,buffer,8);n=int.from_bytes(ext,"big")
    assert not (h[1]&0x80)
    data,buffer=take(s,buffer,n)
...
def connect_ws(port,token):
    ...
    request=(f"GET /ws HTTP/1.1\r\nHost: 127.0.0.1:{port}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: {key}\r\nSec-WebSocket-Version: 13\r\nSec-WebSocket-Protocol: hwm-v1, hwm-bearer.{token}\r\nOrigin: chrome-extension://integration-test\r\n\r\n").encode();s.sendall(request)
    received=recv_until(s,b"\r\n\r\n");header_bytes,buffer=received.split(b"\r\n\r\n",1);headers=(header_bytes+b"\r\n\r\n").decode("ascii");assert "101 Switching Protocols" in headers,headers
    expected=base64.b64encode(hashlib.sha1((key+GUID).encode()).digest()).decode();assert f"Sec-WebSocket-Accept: {expected}" in headers,headers;assert "Sec-WebSocket-Protocol: hwm-v1" in headers;return s,buffer
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            ws,buffer=connect_ws(port,token);first,buffer=recv_frame(ws,buffer);assert first["type"]=="state" and first["status"]["revision"]==0,first
            assert req(base,"/debug/demo-state","POST",{},token)[0]==200
            pushed,buffer=recv_frame(ws,buffer);assert pushed["type"]=="state",pushed;st=pushed["status"];assert st["revision"]>=1 and st["state_hash"] and st["side_to_act"]==1 and st["active_entity_uid"],st
