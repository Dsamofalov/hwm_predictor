...
function Insert-After-Heading([string]$Path, [string]$Heading, [string]$Marker, [string]$Paragraph) {
  $text = Read-Lf $Path
  if ($text.Contains($Marker)) { throw "$Path: marker already exists unexpectedly: $Marker" }
  $h = $text.IndexOf($Heading)
  if ($h -lt 0) { throw "$Path: missing heading: $Heading" }
  $e = $text.IndexOf("`n", $h)
  if ($e -lt 0) { throw "$Path: heading has no line ending: $Heading" }
  $updated = $text.Substring(0, $e + 1) + "`n" + $Paragraph.Trim() + "`n" + $text.Substring($e + 1)
...
function Commit-Paths([string]$Message, [string[]]$Paths) {
  foreach ($path in $Paths) {
    git add -- $path
    if ($LASTEXITCODE -ne 0) { throw "git add failed for $path ($Message)" }
  }
  $staged = @(git diff --cached --name-only)
  if ($LASTEXITCODE -ne 0) { throw "git diff --cached --name-only failed: $Message" }
  if ($staged.Count -eq 0) { throw "No staged change for: $Message" }
...
$scriptUpdateCommit = (git rev-parse HEAD^).Trim()

# 1. Synchronize TEST_REPORT.md with the last fully validated functional tree.
...
$reportSha = Commit-Paths 'docs: synchronize test report with current Windows CI' @('TEST_REPORT.md')

$syncLog = @"
### Current Windows CI / report synchronization

- Aborted first-pass staging commits: `a8d24e7bccc297d124d15e21e1e61332510af97a` and `bca05a03faa07e54385bfd0ee8482c52b54ec0fd`.
  - The first staged an over-broad draft synchronizer; the second removed it before execution. No specification content was changed by that attempt.
- One-shot synchronization setup commits: `ca984c154f1357786966bfebf7067fbb1028d212`, `6534ee7eb5f493252b1a58f76d514fcf0d0546de`, `cb6658b9326a27231b34de2d69f94a6890304456`, `$scriptUpdateCommit` and `$workflowCommit`.
  - The final two are the hardened script revision and the triggering workflow revision. Temporary files are removed by the specification commit below.
- Functional test reference: `7cd17878174529a40087ce5a78231dd93690851b`.
  - Bounded Windows daemon cold-start tolerance; workflow `31431838319`: **Core PASS + Full PASS**.
  - Core: MSVC Debug main-front CTest, 120/120 held-out planner validity, four daemon integrations, Python 75/75, TypeScript/extension PASS.
  - Full: MSVC Release main-front CTest, planner benchmark and all permanent M11 evidence commands PASS.
- Commit: `$reportSha`.
...
$syncLogSha = Commit-Paths 'docs: record current Windows CI synchronization' @('changelog.md')

# 2. Refresh both copies of the active specification from work completed since 144d958.
...
$mvpNote = '> **Checkpoint 11.08.2026:** replay-часть пункта 10 уже закрыта permanent gate **120/120** с 0 invalid recommendations. Общий MVP всё ещё **не COMPLETE**, потому что пункты live acquisition/closed-loop должны быть подтверждены реальным authenticated active-battle smoke; hard-PvE quality относится к следующему milestone.'
...
foreach ($required in @('Python **75/75**', '120/120', 'ADVANCED PARTIAL / EXPERIMENTAL', 'MOSTLY COMPLETE FOR PLUMBING / LIVE NOT VALIDATED', 'Production learned dynamics remains **disabled**', 'conservative exact re-root', 'Evaluation Harness')) {
...
foreach ($stale in @('Planner real-state regression (Release, 20 states)', 'Persistent search-tree re-rooting по predicted child ещё не завершён.', 'Tree reuse/transpositions и более сильная stochastic opponent branching ещё в развитии.')) {
  if ($specText.Contains($stale)) { throw "SPEC validation found stale implementation statement: $stale" }
}

Remove-Item '.github/scripts/docs_checkpoint_sync.ps1' -Force
Remove-Item '.github/workflows/docs_checkpoint_sync.yml' -Force
$specSha = Commit-Paths 'docs: refresh active specification checkpoint' @('SPEC.md', 'HeroesWM_Solver_TZ_Status_0.3.0.md', '.github/scripts/docs_checkpoint_sync.ps1', '.github/workflows/docs_checkpoint_sync.yml')

$specLog = @"
### Active specification refresh after M13/M11/Windows CI work

- Commit: `$syncLogSha`.
  - Recorded the test-report synchronization and exact functional-tree validation evidence.
- Commit: `$specSha`.
  - Updated both `SPEC.md` and `HeroesWM_Solver_TZ_Status_0.3.0.md` to checkpoint date 11.08.2026 and kept them byte-identical.
  - Recorded Windows-only self-hosted Core/Full CI and Python 75/75; historical Linux references are explicitly non-permanent evidence/portability goals.
  ...
  - Updated M03/M13/M14 for stochastic outcome separation, transpositions, persistent exact re-root, structure fingerprint, scheduler-recency hashing, semantic-effect provenance canonicalization and closed-loop reuse guards.
  ...
  - Updated M19/Phase 9/MVP DoD to mark the >=100-state replay recommendation gate closed at 120/120 while retaining authenticated live shadow and hard-PvE quality as open gates.
  - Updated Phase 6/Search and Phase 8/online-advisor without claiming the still-missing authenticated active-battle smoke.
  - Reordered main-front priorities: live smoke and decoder/legal >=99.9% remain blockers; stronger opponent/chance branching and structured learned dynamics are subsequent quality fronts.
  - Removed the temporary synchronization script/workflow.
"@
Append-Text 'changelog.md' $specLog
$finalSha = Commit-Paths 'docs: record specification checkpoint refresh' @('changelog.md')
