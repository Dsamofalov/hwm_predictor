...
def test_geometric_mean_profile_averages_log_residuals():
    profile = tg.GeometricMeanProfile((_Profile(0.5), _Profile(2.0), _Profile(8.0)))
    assert profile.multiplier("MELEE_ATTACK", 1) == pytest.approx(2.0)
    assert tg.GeometricMeanProfile(tuple()).multiplier("MELEE_ATTACK", 1) == pytest.approx(1.0)
...
def test_summarize_accepts_nondefault_horizon_keys():
    metrics = {
        "3": {
            "windows": 2,
            "learned_mean_force_l1": 0.1,
            "generic_mean_force_l1": 0.2,
            "learned_mean_valid_observed_action_coverage": 0.99,
            "generic_mean_valid_observed_action_coverage": 0.98,
        }
    }
    summary = tg.summarize(metrics)
    assert summary["l1_better_than_generic_at_all_horizons"]
    assert summary["coverage_not_below_generic_at_all_horizons"]
    assert summary["mean_l1_ratio_learned_over_generic"] == pytest.approx(0.5)
